...
export async function GET() {
  try {
    const [trending, newest] = await Promise.all([
      fetchJson(
        "https://api.geckoterminal.com/api/v2/networks/robinhood/trending_pools?page=1",
      ).catch(() => ({ data: [], included: [] })),
      fetchJson(
    ...
    for (const pool of pools) {
      ...
      launches.push({
        ...
        mcap: fmtUsd(mcap),
        status,
        logo: token?.attributes?.image_url || "/images/hood-rpc/tokens/rh.png",
        pairUrl: `https://www.geckoterminal.com/robinhood/pools/${addr}`,
        createdAt: created,
      });
    }
