...
/** Optional — register slash commands to one guild instantly (dev). */
...
/**
 * Optional comma-separated Discord channel IDs to enable on boot when the
 * store has no buybot channels yet.
 * Example: BUYBOT_SEED_CHANNELS=1234567890123456789
 */
export const BUYBOT_SEED_CHANNELS: string[] = (
  process.env.BUYBOT_SEED_CHANNELS ?? ""
)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
export function requireToken(): string {
  if (!DISCORD_BOT_TOKEN) {
    throw new Error(
      "DISCORD_BOT_TOKEN is missing. Copy .env.example → .env and paste your Discord bot token.",
    );
  }
...
export function requireClientId(): string {
  if (!DISCORD_CLIENT_ID) {
    throw new Error(
      "DISCORD_CLIENT_ID is missing. Set it to your Discord application (bot) client ID.",
    );
  }
